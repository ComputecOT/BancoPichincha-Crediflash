...
char matriz[5000][2000], matrizM[5000][2000], Aceptacion[5], Cuenta[30],Direccion[200],
	  DirChar[60], medioEnvio[5];
...
//					if(k==0 && matriz[k][0]=='1' &&  matriz[k][1]=='A')
			...
			memset(medioEnvio,'\0',sizeof(medioEnvio));
			memcpy(medioEnvio,linea+137,3);
			elim_esp_izq(medioEnvio);
			CortaCad(medioEnvio);
			if(atoi(medioEnvio)==1){
				linea[1055]='N';
			}else if(atoi(medioEnvio)==2){
				linea[1055]='S';
			}else{
				linea[1055]='A';
			}
